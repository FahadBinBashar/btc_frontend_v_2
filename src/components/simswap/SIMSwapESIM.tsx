...
interface SIMSwapESIMProps {
  phoneNumber: string;
  requestId: string | number;
  onComplete: () => void;
}

const SIMSwapESIM = ({ phoneNumber, requestId, onComplete }: SIMSwapESIMProps) => {
  ...
  useEffect(() => {
    if (!requestId) return;
    api.simswapFinalizeEsim(requestId).catch((err) => {
      console.error("Failed to finalize eSIM:", err);
    });
  }, [requestId]);
  ...
  const saveToGallery = async () => {
    ...
    try {
      ...
      ctx.fillText("BTC SIM Swap - eSIM", canvas.width / 2, 40);
      
      ctx.font = "16px sans-serif";
      ctx.fillText(`Number: +267 ${phoneNumber}`, canvas.width / 2, 70);
      ...
      canvas.toBlob((blob) => {
        if (blob) {
          ...
          a.href = url;
          a.download = `BTC-SIMSwap-eSIM-${phoneNumber}.png`;
    ...
    return (
      ...
      >
        <div className="bg-card border border-success/30 rounded-xl p-8 text-center">
          ...
          <h2 className="text-2xl font-bold text-foreground mb-2">SIM Swap Complete!</h2>
          <p className="text-muted-foreground mb-6">
            Your number has been successfully transferred to your new eSIM.
          </p>
          ...
          <div className="space-y-3 text-left bg-muted/50 rounded-lg p-4 mb-6">
            ...
            <ul className="space-y-2 text-sm text-muted-foreground">
              <li className="flex items-center gap-2">
                ...
                <span>Your old SIM has been deactivated</span>
              </li>
              <li className="flex items-center gap-2">
                <CheckCircle2 className="w-4 h-4 text-success" />
              ...
              <li className="flex items-center gap-2">
                <CheckCircle2 className="w-4 h-4 text-success" />
                <span>All your existing services are active</span>
              </li>
            </ul>
          </div>
      ...
      <div className="bg-card border border-border rounded-xl p-6 mb-6">
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-muted-foreground">
            Scan the QR code below to activate your new eSIM
          </p>
        </div>
